...
export const getHGLeagueListAllByToken = toAsyncTimeFunction(async function (url, uid, ver, count) {
    if (count !== undefined && count < 0)
        throw Error('getLeagueListAllByNodeFetch 递归太多');
    const body = {
    ...
    const mixObj = Convert.xml2js(text, { compact: true });
...
export const getHGGameListByTokenAndLeagueId = toAsyncTimeFunction(async function (url, ver, uid, lid, count) {
    if (count !== undefined && count < 0)
        throw Error('getGameListByTokenAndLeagueId 递归太多');
    const body = {
        uid: uid,
        ver: ver,
        langx: 'zh-cn',
...
export const getHGGameOBTByTokenAndEcid = toAsyncTimeFunction(async function (url, ver, uid, ecid, count) {
    ...
    const text = await res.text();
    let mixObj = Convert.xml2js(text, { compact: true });
    if (mixObj?.serverresponse?.code?._text === 'error') {
        await reLogin(uid);
...
export const getJCInfoList = toAsyncTimeFunction(async function (count = 5) {
    ...
    const res = await cuFetch('https://webapi.sporttery.cn/gateway/jc/football/getMatchCalculatorV1.qry?poolCode=hhad,had,ttg,hafu&channel=c', {
        headers: {
    ...
    const JCInfoList = (matchInfoList ?? [])
        ...
        return {
            ...
            isSingle_had: (match?.poolList || []).find((pool) => pool.poolCode === 'HAD')?.single?.toString?.() ?? '',
            isSingle_hhad: (match?.poolList || []).find((pool) => pool.poolCode === 'HHAD')?.single?.toString?.() ?? '',
            had_a: match?.had?.a ?? '',
            had_h: match?.had?.h ?? '',
            had_d: match?.had?.d ?? '',
            hhad_a: match?.hhad?.a ?? '',
